...
{
    public GameObject leftScript;
    public GameObject rightScript;
    public GameObject lcube, rcube;
    ...
    void Update()
    {
        lcube.transform.position = leftScript.GetComponent<Leap.Unity.CapsuleHand>()._spherePositions[0];
        rcube.transform.position = rightScript.GetComponent<Leap.Unity.CapsuleHand>()._spherePositions[0];
        Debug.Log("Left Hand Position: " + lcube.transform.position.ToString() + ", Right Hand Position: " + rcube.transform.position.ToString());
    }
}
